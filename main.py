...
def mains():
    ...
    if IN_GAME:
        ...
        if head_coords not in blocklist:
            create_tail(head_coords)
    ...
    def __init__(self):
        ...
        for i in range(0, 9):
            if 0 <= i <= 2:
                ...
                blocklist.append(c.coords(c.create_rectangle(x1 + g, y1, x2 + g, y2,
                                   fill="green")))
                g = g + 10
            elif 3 <= i <= 5:
                if i == 3:
                    blocklist.append(c.coords(c.create_rectangle(x1, y1 + 10, x2, y2 + 10,
                                       fill="green")))
                    y1 += 10
                    y2 += 10
                    g = 0
                ...
                if i == 6:
                    y1 += 10
                    y2 += 10
                    blocklist.append(c.coords(c.create_rectangle(x1, y1, x2, y2,
                                       fill="green")))
                    g = 0
                else:
                    g = g + 10
